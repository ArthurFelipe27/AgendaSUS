...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ... (As funções helper 'DIAS_SEMANA_MAP' e 'getProximaDataISO' continuam aqui, sem alterações) ...
    ...
    // ... (Todas as outras funções como renderMeuPerfil, renderListaMedicos, handleBookingSubmit, etc. continuam aqui) ...
    // A única função que vamos substituir é a handleVerDetalhesConsulta.

    // Vou colar o arquivo inteiro abaixo para garantir que não haja erros.
        ...
        try {
            ...
            if (proximosAgendamentos.length === 0) {
                ...
                proximosAgendamentos.forEach(ag => {
                    html += `<div class="agendamento-card status-${ag.status}"><div><strong>${new Date(ag.dataHora).toLocaleString('pt-BR', { dateStyle: 'long', timeStyle: 'short' })}</strong><br><small>Médico(a): ${ag.medico.nome} | Status: ${ag.status}</small></div><button class="btn btn-danger btn-cancelar-agendamento" data-id="${ag.idAgendamento}">Cancelar</button></div>`;
                });
            }
     ...
     * Lida com respostas de API que podem vir vazias, sem quebrar o JSON.
     */
    async function handleVerDetalhesConsulta(agendamentoId) {
        try {
            ...
            // Lógica robusta para parsear JSON apenas se a resposta não for vazia
    ...
    async function renderMinhasPrescricoes() {
        // Esta função agora é um atalho para a tela de agendamentos
        renderMeusAgendamentos();
    }
