// ===================================================================
// PACIENTE.JS (VERSÃO FINAL COM TODAS AS CORREÇÕES)
// ===================================================================
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function initPacienteDashboard() {
        try {
            ...
            if (!responseMe.ok) throw new Error('Falha ao buscar perfil.');
            const pacienteUser = await responseMe.json();
            idUsuarioLogado = pacienteUser.id;
        } catch (e) {
            console.error(e);
            contentArea.innerHTML = "<p>Erro fatal ao carregar dados.</p>";
            return;
        }
    ...
    // --- RENDER VIEWS ---
    async function renderMeuPerfil() { /* ... implementação que já temos ... */ }
    async function renderListaMedicos() { /* ... implementação que já temos ... */ }
    async function renderDetalhesMedicoParaAgendamento(medicoId) { /* ... implementação que já temos ... */ }
    function renderFormularioSintomas(dataISO, medicoId, nomeMedico) { /* ... implementação que já temos ... */ }
    async function handleBookingSubmit(event, dataISO, medicoId) { /* ... implementação que já temos ... */ }
    async function renderMeusAgendamentos() { /* ... A versão corrigida está abaixo ... */ }
    async function handleCancelarAgendamento(agendamentoId) { /* ... implementação que já temos ... */ }
    async function handleVerDetalhesConsulta(agendamentoId) { /* ... A versão corrigida está abaixo ... */ }

    // --- COLE AQUI TODAS AS FUNÇÕES COMPLETAS DO PACIENTE.JS ---

    // Vou colar abaixo para garantir 100% de correção
    async function renderMeuPerfil() {
        ...
        } catch (err) { document.getElementById('perfil-info').innerHTML = '<p>Erro ao carregar perfil.</p>'; }
        ...
        try {
            ...
            if (proximosAgendamentos.length === 0) {
                ...
                proximosAgendamentos.forEach(ag => {
                    html += `<div class="agendamento-card status-${ag.status}"><div><strong>${new Date(ag.dataHora).toLocaleString('pt-BR', { dateStyle: 'long', timeStyle: 'short' })}</strong><br><small>Médico(a): ${ag.medico.nome} | Status: ${ag.status}</small></div><button class="btn-delete btn-cancelar-agendamento" data-id="${ag.idAgendamento}">Cancelar</button></div>`;
                });
            }
            ...
            if (historicoAgendamentos.length === 0) {
                ...
                historicoAgendamentos.forEach(ag => {
                    html += `<div class="agendamento-card status-${ag.status}"><div><strong>${new Date(ag.dataHora).toLocaleString('pt-BR', { dateStyle: 'long', timeStyle: 'short' })}</strong><br><small>Médico(a): ${ag.medico.nome} | Status: ${ag.status}</small></div>${ag.status === 'ATENDIDO' ? `<button class="btn-primary-doc btn-ver-detalhes" data-id="${ag.idAgendamento}">Ver Detalhes</button>` : ''}</div>`;
                });
            }
